...
class AddSubCommand extends SubCommand {
    ...
    public  function execute(CommandSender $sender, array $args): bool {
        ...
        if ($this->getPlugin()->addMoney($player, $args[1], $sender)) {
            $sender->sendMessage(str_ireplace(["{ISSUER}", "{AMOUNT}", "{PLAYER}"], [$sender->getName(), $args[1], $player], $this->getPlugin()->getMessage("add-command")));
            $player->sendMessage(str_ireplace(["{ISSUER}", "{AMOUNT}", "{PLAYER}"], [$sender->getName(), $args[1], $player], $this->getPlugin()->getMessage("add-command")));
            return true;
        } else {
